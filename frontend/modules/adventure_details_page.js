...
function addBootstrapPhotoGallery(images) {
  ...
  <div class="carousel-indicators" id="slides-indicator">
  ...
  images.forEach((src, i) =>{
    ...
    document.getElementById('slides-indicator').innerHTML +=`<button type="button" data-bs-target="#carouselExampleIndicators" data-bs-slide-to=${i} class="${i === 0 ? " active" : "" }" aria-current="true" aria-label="Slide ${i+1}"></button>`; 
  })
}
...
function conditionalRenderingOfReservationPanel(adventure) {
  ...
  // 1. If the adventure is already reserved, display the sold-out message.
    console.log("conditional", adventure);
...
function calculateReservationCostAndUpdateDOM(adventure, persons) {
  ...
  // 1. Calculate the cost based on number of persons and update the reservation-cost field
    // console.log("cost reservation", adventure,persons);
...
function captureFormSubmit(adventure) {
  ...
  // console.log("captureform", adventure);
...
function showBannerIfAlreadyReserved(adventure) {
  ...
  // 1. If user has already reserved this adventure, show the reserved-banner, else don't
    console.log("alreadybooked", adventure);
